...
class Table extends \ADIOS\Core\ViewWithController
{
  ...
  private \ADIOS\Core\ViewWithController $paging;
  ...
  public function render(string $panel = ''): string
  {
    ...
    if ($this->params['showPaging']) {
      $this->paging->addView('\\ADIOS\\Core\\ViewsWithController\\Button', [
      ...
      for ($i = 1; $i <= $this->pagesCount; ++$i) {
        if ($i == $this->params['page']) {
          $this->paging->addView('\\ADIOS\\Core\\ViewsWithController\\Html', ["html" => "
              <input
                type='text'
    ...
    if (!$this->params['refresh']) {
      ...
      if ($this->params['showTitle']) {
        ...
        foreach ($this->params['leftTitleButtons'] as $buttonKey => $buttonParams) {
          $titleLeftContent[] = $this->addView('\\ADIOS\\Core\\ViewsWithController\\Button', $buttonParams);
        }
